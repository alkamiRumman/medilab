...
if (localStorage.getItem('addQuote')) {
	let data = JSON.parse(localStorage.getItem('addQuote'));
	for (let key in data) {
		pil[key] = data[key];
		if (key === 'itemList') {
...
$("#quoteNote").on('change', function () {
	pil.quoteNote = $(this).val();
	saveToLocal();
});
...
function saveToLocal() {
	localStorage.setItem("addQuote", JSON.stringify(pil));
}
...
$("#submitForm").on('click', function () {
	...
	if (pil.customer && Object.keys(pil.itemList).length > 0) {
		$.post($("#quoteForm").attr("action"), pil, null, "json")
			.done(function (body, status, req) {
				if (body.status) {
					toastr.success("Quote added!<br>Now redirecting to print receipt!", "Quote Added Successfully!");
					localStorage.removeItem("addQuote");
					setTimeout(function () {
						open(globalSetting.baseUrl + "/quote", "_self");
					}, 1000);
				} else {
					toastr.error("Something went wrong!<br>Please Try Again!", "Quote Not Saved!");
				}
			})
			.fail(function (e, t, y) {
				console.log(e, t, y);
