...
let dateTime = $("#dateTime").daterangepicker({
	...
	startDate: pick.date ? globalSetting.dbDateTimeFormatter(pick.date) : new Date()
});
dateTime.on('change', function () {
	pick.date = $("#dateTime").val();
	saveToLocal();
});

var $vendorSelectize = $("#vendor").selectize({
	...
	load: function (query, callback) {
		$.ajax({
			dataType: "json",
			type: 'post',
			url: globalSetting.baseUrl + "/peoples/getVendors",
			data: {term: query},
			error: function () {
				callback();
			},
	...
	onItemAdd: function (value, item) {
		let vendor = $vendorSelectize[0].selectize.options[value];
		delete vendor.$order;
		pick.vendor = vendor;
		pick.date = $("#dateTime").val();
		saveToLocal();
...
var $productSltz = $("#product").selectize({
	...
	load: function (query, callback) {
		$.ajax({
			...
			data: {term: query, excludes: pick.excludes}
		}).fail(function (err) {
			console.log(err);
			callback(err);
		}).done(function (res) {
...
if (localStorage.getItem('addPurchase')) {
	let data = JSON.parse(localStorage.getItem('addPurchase'));
	for (let key in data) {
		pick[key] = data[key];
		if (key === 'itemList') {
	...
	$("#purchaseNote").val(pick.purchaseNote);
	$("#staffNote").val(pick.staffNote);
	$("#orderDiscount").val(pick.orderDiscount.rate);
	if (pick.orderDiscount.type) {
		$("#orderDiscountType").prop('checked', true);
	}
	if (pick.orderTax.id) {
		$("#orderTax").val(pick.orderTax.id);
	}
	if (pick.vendor) {
		$vendorSelectize[0].selectize.addOption(pick.vendor);
		$vendorSelectize[0].selectize.setValue(pick.vendor.id, true);
	}
	updateTotal();
}
...
function updateTotal() {
	let productTotal = pick.getTotal();
	...
	if (!$("#orderDiscountType").is(":checked")) {
		...
		discount.type = true;
		discount.amount = Number($("#orderDiscount").val()) * Number(pick.total) / 100;
	}
	pick.orderDiscount = discount;
	...
	tx.amount = (tx.percent ? (tx.rate / 100 * pick.subTotal) : tx.rate);
	pick.orderTax = tx;
	productTotal = pick.getTotal();
	...
	//
	$("#totalItem").text(pick.itemType + " [" + parseInt(pick.item) + "]");
	$("#productTotal").text(globalSetting.currencyFormat(pick.total));
	$("#orderDiscountTotal").html(globalSetting.currencyFormat(pick.orderDiscount.amount));
	$("#orderSubTotal").html(globalSetting.currencyFormat(pick.subTotal));
	$("#orderTaxTotal").html(globalSetting.currencyFormat(pick.orderTax.amount));
	$("#grandTotal").html(globalSetting.currencyFormat(pick.grandTotal));
	saveToLocal();
}
...
function removeRowByID($id) {
	...
	pick.excludes.pop($id);
	delete pick.itemList[$id];
	updateTotal();
}
...
$("#purchaseNote").on('change', function () {
	pick.purchaseNote = $(this).val();
	saveToLocal();
});
$("#staffNote").on('change', function () {
	pick.staffNote = $(this).val();
	saveToLocal();
});

function saveToLocal() {
	localStorage.setItem("addPurchase", JSON.stringify(pick));
}
...
$("#submitForm").on('click', function () {
	if (pick.vendor && Object.keys(pick.itemList).length > 0) {
		$.post($("#purchaseForm").attr("action"), pick, null, "json")
			.done(function (body, status, req) {
				console.log(body.status);
				if (body.status) {
					toastr.success("Purchase added!<br>Now redirecting to print receipt!", "Purchase Added Successfully!");
					localStorage.removeItem("addPurchase");
					setTimeout(function () {
						open(globalSetting.baseUrl + "/purchase", "_self");
					}, 1000);
				} else {
					console.log(pick);
					toastr.error("Something wentc wrong!<br>Please Try Again!", "Purchase Not Saved!");
				}
			})
			.fail(function (e, t, y) {
				console.log(e, t, y);
				toastr.error("Something went wrong!<br>Please Try Again!", "Purchase Not Saved!");
			});
	} else {
...
function viewVendor() {
	let $vendor = $("#vendor");
	if ($vendor.val()) {
		showPopup("#remoteModal1", globalSetting.baseUrl + '/peoples/vendor/' + $vendor.val());
	} else {
		toastr.warning("No vendor Selected!", "Not Found!");
	}
}