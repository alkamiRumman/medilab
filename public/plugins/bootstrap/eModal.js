...
(function (define) {
    define(['jquery'], function ($, root) {
        ...
        var lastParams = {};
        var MODAL_BODY = 'modal-body py-2';
        ...
        var defaultSettings = {
            ...
            labels: LABEL,
            loadingHtml: '<h5><i class="fas fa-spinner fa-pulse"></i>Loading...</h5><div class=progress><div class="progress-bar progress-bar-striped active" style="width: 100%"></div></div>',
            size: EMPTY,
            title: 'Attention',
            autofocus: false
        };
        ...
        function _getFooter(buttons) {
            ...
            var messageFotter = $(DIV).addClass('modal-footer py-1').prop('id', FOOTER_ID);
            if (buttons) {
        ...
        function _getModalInstance(skipEventChageIfExists) {
            ...
            function createModalElement() {
                return $('<div class="modal fade"  data-backdrop="static" data-keyboard="false" tabindex="-1" style="z-index: 99999; background: rgba(1,1,1,.5);"><style>.modal-xl{width:96%;}.modal-body{max-height: calc(100vh - 145px);overflow-y: auto;}</style>' +
                    '<div class=modal-dialog>' +
                    '<div class=modal-content>' +
                    ' <div class="modal-header py-2"><h5 class=modal-title></h5><button type=button class="x close" data-dismiss=modal aria-label="Close"><span aria-hidden=true>&times;</span></button></div>' +
                    '</div>' +
                    '</div>' +
                    '</div>')
        ...
        function iframe(params, title) {
            ...
            var html = ('<div class="modal-body py-2" style="position: absolute;width: 100%;background-color: rgba(255,255,255,0.8);height: 100%;">%1%</div>' +
